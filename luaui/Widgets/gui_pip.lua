...
local function DeleteSeismicPingDlists()
	for i = 0, 3 do
		if seismicPingDlists.outerArcs[i] then
			gl.DeleteList(seismicPingDlists.outerArcs[i])
		end
		if seismicPingDlists.outerOutlines[i] then
			gl.DeleteList(seismicPingDlists.outerOutlines[i])
		end
	end
	for i = 0, 2 do
		if seismicPingDlists.middleArcs[i] then
			gl.DeleteList(seismicPingDlists.middleArcs[i])
		end
		if seismicPingDlists.middleOutlines[i] then
			gl.DeleteList(seismicPingDlists.middleOutlines[i])
		end
	end
	for i = 0, 1 do
		if seismicPingDlists.innerArcs[i] then
			gl.DeleteList(seismicPingDlists.innerArcs[i])
		end
		if seismicPingDlists.innerOutlines[i] then
			gl.DeleteList(seismicPingDlists.innerOutlines[i])
		end
	end
	if seismicPingDlists.centerCircle then
		gl.DeleteList(seismicPingDlists.centerCircle)
	end
end
...
local function GL4DrawIcons(checkAllyTeamID, selectedSet, trackingSet)
	...
	if not bldgBlockValid and bldgBlock and bCount > 3000 and checkAllyTeamID == gl4Icons._bldgBlockCheckAlly and (currentGameFrame - (gl4Icons._bldgBlockFrame or 0)) < bldgBlockGameFrameLimit and not useUnitpics and not gl4Icons._bldgBlockBuiltDuringUnitpics and (gl4Icons._bldgRenderFrame - (gl4Icons._bldgBlockRebuildRenderFrame or 0)) < 5 then
		bldgBlockValid = true
	end
...
local function UpdateR2TUnits(currentTime, pipUpdateInterval, pipWidth, pipHeight)
	...
	local shouldUpdate = pipR2T.unitsNeedsUpdate or rotChanged or driftForced or (sizeChanged and not uiState.areResizing) or effectiveUpdateInterval == 0 or (effectiveUpdateInterval > 0 and timeSinceLastUpdate >= effectiveUpdateInterval)
...
local function UpdateR2TCheapLayers(currentTime, pipUpdateInterval, pipWidth, pipHeight)
	...
	local shouldUpdate = pipR2T.contentNeedsUpdate or rotChanged or driftForced or (sizeChanged and not uiState.areResizing) or pipUpdateInterval == 0 or (pipUpdateInterval > 0 and timeSinceLastUpdate >= pipUpdateInterval)
...
local function CreateIconShatter(unitID, unitDefID, unitTeam, unitVelX, unitVelZ)
	...
	if iconCount >= 4000 then
		return
	end -- no shatters at all above 4000 icons
	if iconCount >= 3000 then
		...
		if not xs or (xs < 16 and (not zs or zs < 16)) then
			return
		end
	end
